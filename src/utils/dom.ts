...
export function isValidDims(dims: Dims): boolean {
  ...
  if ((!dims.height && dims.height !== 0) || (!dims.width && dims.height !== 0)) {
    return false;
  }
...
export function getNearestScrollAncestor(element: Element): Element {
  ...
  const style = (el: Element, prop: string) => getComputedStyle(el, null).getPropertyValue(prop);

  const scroll = (el: Element) => regex.test(style(el, 'overflow') + style(el, 'overflow-y') + style(el, 'overflow-x'));
...
export function isWithinAt(aDims: Dims, bDims: Dims, aCoords?: Coords, bCoords?: Coords) {
  ...
  const fitsHorizontally: boolean =
    validCoordsA.x >= validCoordsB.x && validCoordsA.x + aDims.width <= validCoordsB.x + bDims.width;
  const fitsVertically: boolean =
    validCoordsA.y >= validCoordsB.y && validCoordsA.y + aDims.height <= validCoordsB.y + bDims.height;
